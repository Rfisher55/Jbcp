...
function lngToZone(lng) { return Math.floor((lng + 180) / 6) + 1; }
function toUTM(lat, lng, zone) { return proj4('WGS84', utmProj(zone, lat >= 0), [lng, lat]); }
function fromUTM(e, n, zone, north = true) { return proj4(utmProj(zone, north), 'WGS84', [e, n]); }
function toMGRS(lat, lng, precision = 5) { try { return mgrs.forward([lng, lat], precision); } catch { return null; } }
...
function parseMGRS(raw) {
  ...
  if (/^\d{4,10}$/.test(s) && s.length % 2 === 0) {
    ...
    try {
      ...
      if (isFinite(lat) && isFinite(lng)) return { lat, lng, valid: true, note: `Expanded to ${full}` };
    } catch {}
  }
  return { valid: false };
}
...
const MGRSGrid = L.Layer.extend({
  ...
  _draw() {
    ...
    for (let lng = Math.floor(w / 6) * 6; lng <= e; lng += 6) zones.add(lngToZone(lng));
    zones.add(lngToZone(w)); zones.add(lngToZone(e));
    for (const z of zones) this._drawZone(ctx, map, bounds, z, interval, digits);
  },
  _drawZone(ctx, map, bounds, zone, interval, digits) {
    const zW = (zone - 1) * 6 - 180, zE = zone * 6 - 180;
    const latS = Math.max(bounds.getSouth(), -80), latN = Math.min(bounds.getNorth(), 84);
    const lngW = Math.max(bounds.getWest(), zW), lngE = Math.min(bounds.getEast(), zE);
    if (lngW >= lngE) return;
    const isN = latS >= 0;
    let eSW, nSW, eNE, nNE, eNW, eSE;
    try {
      [eSW, nSW] = toUTM(latS, lngW, zone); [eNE, nNE] = toUTM(latN, lngE, zone);
      [eNW] = toUTM(latN, lngW, zone); [eSE] = toUTM(latS, lngE, zone);
    } catch { return; }
    ...
    for (let e = eMin; e <= eMax; e += interval) { if (e < 100000 || e > 900000) continue; this._line(ctx, map, zone, isN, 'e', e, nMin, nMax, digits); }
    for (let n = nMin; n <= nMax; n += interval) { if (n < 0 || n > 10000000) continue; this._line(ctx, map, zone, isN, 'n', n, eMin, eMax, digits); }
  },
  _line(ctx, map, zone, isN, axis, fixed, vMin, vMax, digits) {
    ...
    for (let i = 0; i <= steps; i++) {
      ...
      try {
        const [lng, lat] = axis === 'e' ? fromUTM(fixed, v, zone, isN) : fromUTM(v, fixed, zone, isN);
    ...
    ctx.stroke();
    if (digits > 0) {
      const mid = pts[Math.floor(pts.length / 2)], size = map.getSize();
      if (mid.x > 16 && mid.x < size.x - 16 && mid.y > 16 && mid.y < size.y - 16) {
        ...
        ctx.save(); ctx.font = '9px SF Mono,monospace'; ctx.fillStyle = 'rgba(70,210,255,0.8)';
        ctx.fillText(label, mid.x + 2, mid.y - 2); ctx.restore();
      }
    }
  }
});
