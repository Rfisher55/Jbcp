...
const MapCtrl = {
  ...
  fitToUnits() {
    ...
    // Include BFT tracks
    Object.values(BFT._tracks).forEach(t => {
      if (isFinite(t.lat) && isFinite(t.lng)) pts.push([t.lat, t.lng]);
    });
    if (!pts.length) { UI.toast('No units or tracks with positions', 'info'); return; }
